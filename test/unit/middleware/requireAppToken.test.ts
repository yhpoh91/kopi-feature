...
import { requireAppToken } from '../../../src/middleware/requireAppToken.js'
...
describe('requireAppToken', () => {
  it('returns 501 Not Implemented', async () => {
    const result = (await requireAppToken(mockEvent, mockContext, mockNext)) as StructuredResult
    expect(result.statusCode).toBe(501)
    expect(JSON.parse(result.body)).toEqual({ error: 'Not implemented' })
  })

  it('does not call next', async () => {
    await requireAppToken(mockEvent, mockContext, mockNext)
